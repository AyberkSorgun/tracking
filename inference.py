    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        noisyDistance = observation # 3
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition() # 5,2
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
               sumPos[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if allPossible.totalCount()==0:
            ...
            count = 0
            while count < self.numParticles:
                belf.append(util.sample(allPossible))
                count += 1
            self.beliefs = belf
    ...
    def elapseTime(self, gameState):
        ...
        #sample code snippet for map
        #items = [1, 2, 3, 4, 5]
        #squared = list(map(lambda x: x**2, items))
        belf = []
        list(map(lambda x: belf.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, x)))), self.beliefs ))
        self.beliefs = belf


       # for pos in self.beliefs:
       #     newPosDist= self.getPositionDistribution(self.setGhostPosition(gameState, pos))
       #     belf.append(util.sample(newPosDist))
       # self.beliefs = belf
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        sum = 0
        self.beliefs = []
        newPositions = list(itertools.product(self.legalPositions, self.legalPositions))
    ...
    def observeState(self, gameState):
        ...
        for pos in self.beliefs:
            initBelf = 1.0
            for gh in range(0, self.numGhosts):
                if noisyDistances[gh] == None:
                    pos = self.getParticleWithGhostInJail(pos, gh)
                else:
                    trueDistance = util.manhattanDistance(pos[gh], pacmanPosition)
                    if emissionModels[gh][trueDistance] > 0:
                        initBelf = initBelf * emissionModels[gh][trueDistance]
                    else:
                        initBelf = 0.0
            allPossible[pos] += initBelf
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.beliefs:
